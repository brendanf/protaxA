...
int compute_cnode_probs_best2(TaxonomyNode *node, int nid, double prevprob, SequenceSet *rseq, Model *m, double **scs, char *seq, double pth, PtrCountItem *ptrcount, int oklen);
...
int compute_cnode_probs_best2(TaxonomyNode *node, int nid, double prevprob, SequenceSet *rseq, Model *m, double **scs, char *seq, double pth, PtrCountItem *ptrcount, int oklen) {
  ...
  for (i=0; i<node[nid].num_cnodes; i++) {
	...
	compute_cnode_probs_best2(node, cid, node[cid].prob, rseq, m, scs, seq, pth, ptrcount, oklen);
    }
  }
...
int main (int argc, char **argv) {
  ...
  */
  ...
  for (i=0; i<iseq->num_seqs; i++) {
    ...
    compute_cnode_probs_best2(taxonomy, 0, 1.0, rseq, model, scs, iseq->seq[i], pth, ptrcount, oklen);
    printf("\n");
  }
