...
int main (int argc, char **argv) {
  ...
  if (argc - optind != 7) {
    ...
    fprintf(stderr,"usage: classify_v1 [-l len] [-r n_rseq] [-i n_iseq] [-t reporting_threshold] [-m min_overlap] taxonomy rseqFASTA taxonomy2rseq modelparameters scalingfile probability_threshold inputFASTA\n");
    exit(0);
  }
  ...
  */
  ...
  for (i=0; i<iseq->num_seqs; i++) {
    start_time = clock();
    compute_distances(rseq, iseq->seq[i], iseq->start[i], iseq->end[i],
